...
export default function AdminLoginPage() {
  ...
  const handleCredentials = async (e) => {
    ...
    try {
      ...
      addToast('Sign-in issue. Falling back to demo mode.', 'info');
      setStep(2);
    } finally {
      setLoading(false);
    }
  ...
  const handle2FA = async (e) => {
    ...
    loginAdmin({ email: form.email, role: 'Admin', name: 'Admin (Demo)' });
    addToast('Admin access granted (demo mode).', 'success');
    nav('/admin/dashboard');
    setLoading(false);
  };
      ...
      <div className="w-full max-w-md animate-slide-up">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8">
          {step === 1 ? (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={form.email}
                    ...
                    placeholder="admin@harborview.lk"
            ...
            <form onSubmit={handle2FA} noValidate className="space-y-5">
              <div className="text-center mb-4">
                ...
                <p className="text-navy-300 text-xs mt-1">Enter the 6-digit code from your authenticator app</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-navy-200 mb-1.5">Authentication Code</label>
                <input type="text" value={form.twofa}
                  onChange={e => setForm(p => ({ ...p, twofa: e.target.value.replace(/\D/g,'').slice(0,6) }))}
                  placeholder="000000" maxLength={6}
                  className="w-full bg-white/5 border border-white/15 rounded-xl px-4 py-3 text-sm text-white text-center tracking-[0.5em]
                             placeholder-navy-400 focus:outline-none focus:border-gold-500/60 focus:ring-2 focus:ring-gold-500/20"/>
              </div>
              ...
              <button type="button" onClick={() => setStep(1)}
                className="w-full text-navy-400 text-xs hover:text-navy-200 transition-colors">
                Back to credentials
              </button>
            </form>
          )}
